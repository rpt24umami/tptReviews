...
describe('React', () => {
  ...
  it('Contains Satisfaction level', () => {
    console.log('Before');
    ...
    wrapper.setProps({
      ...
      expect(wrapper.containsMatchingElement(<div>description</div>)).toEqual(true);
      expect(wrapper.containsMatchingElement(<div>User</div>)).toEqual(true);
    });
    wrapper.setProps({
      review: {
        rating: 1,
  ...
  it('contains stars available to sort', () => {
    const wrapper = shallow(<Sort grades={['1st Grade', '2nd Grade']}/>);
    console.log(wrapper.debug());
    expect(wrapper.containsMatchingElement(<li value='5'>5 stars</li>)).toEqual(true);
  })
});
